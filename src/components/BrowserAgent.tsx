...
export const BrowserAgent: React.FC<BrowserAgentProps> = ({
  ...
  useEffect(() => {
    const handleNavigationMessage = (event: MessageEvent) => {
      const message = validateNavigationMessage(event, iframeRef.current?.contentWindow ?? null, window.location.origin);
      if (!message) return;
      console.log("[LOHZ Browser] Same-origin child iframe navigated to:", message.url);
  ...
  useEffect(() => {
    ...
    console.log(`[LOHZ Browser Hub] Automated Voice Trigger: ${type}`, args);
          ...
          case "browserMediaControl": {
            ...
            if (iframe && iframe.contentWindow) {
              ...
              if (video) {
                ...
                const mediaOrigin = activeTab?.url ? new URL(activeTab.url).origin : "https://www.youtube.com";
    ...
    if (activeTab) {
      ...
      try {
        ...
        if (iframe && iframe.contentDocument) {
          ...
          if (bodyTxt.includes("LOHZ Web Proxy Error") || bodyTxt.includes("Myraa Web Proxy Error") || bodyTxt.includes("Failed loading remote website")) {
  ...
  return (
    <div
      id="lohz-playwright-automation-hud"
              ...
              <div className="flex-1 overflow-y-auto p-8 flex flex-col items-center justify-center space-y-8 select-none text-center scrollbar-none">
                ...
                <div className="p-3 border border-white/5 bg-slate-950/50 rounded-2xl max-w-md w-full">
                  <p className="text-[9px] font-mono text-slate-500 leading-normal uppercase">
                    🌸 COMPANION TIP: Voice command LOHZ to trigger any automation: <br />
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs font-sans text-slate-400 leading-relaxed max-w-md mx-auto">
                      LOHZ automatically detected secure sandbox constraints for <span className="font-mono text-indigo-300 font-semibold">{getCleanTitleFromUrl(activeTab?.url || "")}</span>. 
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs font-sans text-slate-400 leading-relaxed max-w-sm mx-auto font-medium">
                      LOHZ was unable to load <span className="font-mono text-indigo-300 font-semibold">{getCleanTitleFromUrl(activeTab?.url || "")}</span> securely. This website may be offline, spelled incorrectly, or blocking automated server access.
                    </p>
                  </div>
                  ...
                  {!isLocalConnected ? (
                    ...
                    <div className="flex-1 overflow-y-auto space-y-5 pr-1 select-text">
                      <div className="p-3.5 rounded-xl border border-indigo-500/10 bg-indigo-505/10 text-[11px] font-sans leading-normal text-indigo-300">
                        ...
                        <span>If you prefer to command a real, headed Chrome browser on your desktop computer, easily launch LOHZ's local client script!</span>
                      </div>
                      ...
                      <div className="space-y-4 font-mono text-[10px]">
                        ...
                        <div>
                          ...
                          <div className="p-2.5 bg-slate-950 border border-white/5 rounded-lg flex items-center justify-between text-slate-300">
                            ...
                            <button onClick={() => copyToClipboard("npm run agent", "install3")} className="p-1 rounded hover:bg-white/5 text-slate-500 hover:text-white">
                    ...
                    <div className="flex-1 overflow-y-auto font-mono text-[10px] space-y-2 pr-1 select-text scrollbar-thin">
                      {localLogs.length === 0 ? (
                        <div className="text-slate-600 italic">No operations recorded yet. Speaking with LOHZ will generate real terminal logs.</div>
                      ) : (
                        localLogs.map((log) => (
                          <div
                            key={log.id}
              ...
              <div className="p-4 grid grid-cols-1 md:grid-cols-4 gap-4 font-mono text-[10px] tracking-wide text-slate-400 leading-normal max-h-48 overflow-y-auto select-text scrollbar-thin">
                ...
                <div className="md:col-span-4 flex items-center justify-between border-b border-white/5 pb-1.5 mb-1 text-[11px] font-extrabold uppercase text-amber-400">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span>Active LOHZ Browser Diagnostics console</span>
                  </div>
