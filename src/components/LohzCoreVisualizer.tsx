...
export type MyraaEmotion = LohzEmotion;

interface LohzCoreVisualizerProps {
  session: LohzAudioSession | null;
  state: LiveState;
  themeColor: string; // Violet, crimson, emerald, celestial, gold, rose, charcoal
  activeEmotion?: LohzEmotion;
  characterState: "idle" | "thinking" | "talking";
}

export const LohzCoreVisualizer: React.FC<LohzCoreVisualizerProps> = ({
  session,
  state,
  themeColor,
  activeEmotion = "idle",
  ...
  const handleVideoError = (videoName: string) => {
    console.warn(`[LOHZ Web Video] Failed to load video source for: ${videoName}`);
    setHasError(true);
  };
  ...
  // Cursor position tracking hook and 3D Holographic Perspective Tilt
  ...
  // Smooth interpolation loop for 3D holographic tilt depth-effect
  useEffect(() => {
    let frameId: number;
    let currRotateY = 0;
    let currRotateX = 0;

    const smoothTiltLoop = () => {
      // Rotate horizontal: -8deg to +8deg
      const targetRotateY = (targetMouseRef.current.x - 0.5) * 16;
      // Rotate vertical: -6deg to +6deg
      const targetRotateX = -(targetMouseRef.current.y - 0.5) * 12;

      currRotateY += (targetRotateY - currRotateY) * 0.075;
      currRotateX += (targetRotateX - currRotateX) * 0.075;
      ...
      frameId = requestAnimationFrame(smoothTiltLoop);
    };
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
      ...
      {/* 2. Character Videos state crossfade manager (Z-index 10) with 3D Holographic Tilt, Glitch & Chromatic Aberration */}
      <div 
        id="lohz-animated-presence"
        className="absolute z-10 w-full h-full flex items-center justify-center pointer-events-auto transition-all duration-700 animate-holo-glitch"
        style={{
          perspective: "1200px",
      ...
      >
        <div 
          ...
          style={{
            transform: `perspective(1200px) rotateX(${tilt.rotateX.toFixed(2)}deg) rotateY(${tilt.rotateY.toFixed(2)}deg) translateZ(12px)`,
            transformStyle: "preserve-3d",
          }}
        >
          {/* Subtle Outer Ambient Shadow Cast with Parallax depth */}
          <div 
            className="absolute inset-0 rounded-[2.5rem] blur-[30px] opacity-20 bg-cyan-600/15 pointer-events-none mix-blend-screen transition-transform duration-100"
            style={{
              transform: `translate3d(${(-tilt.rotateY * 1.5).toFixed(1)}px, ${(-tilt.rotateX * 1.5).toFixed(1)}px, -15px)`,
            }}
          />

          {/* Holographic vertical scanline sweep overlay */}
          <div className="absolute inset-0 rounded-[2.5rem] overflow-hidden pointer-events-none z-20">
            <div className="w-full h-12 bg-gradient-to-b from-transparent via-cyan-400/15 to-transparent animate-holo-scanline pointer-events-none mix-blend-overlay" />
          </div>
      ...
      <canvas
        id="lohz-hologram-living-canvas"
        ref={canvasRef}
...
export { LohzCoreVisualizer as MyraaCoreVisualizer };
